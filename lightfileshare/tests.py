...
class FilePermissionTestCase(TestCase):
    def setUp(self):
        ...
        file1 = SimpleUploadedFile(
            "dummy-file1.mp4", b"randomFile con-tent.11", content_type="video/mp4"
        )
        ...
        file2 = SimpleUploadedFile(
            "dummy-file2.mp4", b"randomFile con-tent.22", content_type="video/mp4"
        )
        SecretFile.objects.create(
    ...
    def test_file_upload_and_permission(self):
        ...
        self.FPH.check_permission(
            client, SecretFile.objects.get(pk=2).filename, _debug=True
        )
        ...
        assert response.status_code == 302, "status != 302"
        assert (
            r"fail=You%20have%20no%20permission%20to%20access%20this%20file"
            in response.url
        ), f"Err : {response.url}"
