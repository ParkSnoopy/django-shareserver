...
UNSAFES_BASE = set(
    filter(
        bool,
        open(cwd / "localutils/unsafes/base.txt", "r", encoding="utf-8").read().split(),
    )
)
UNSAFES_ROOT = (
    set(
        filter(
            bool,
            open(cwd / "localutils/unsafes/root.txt", "r", encoding="utf-8")
            .read()
            .split(),
        )
    )
...
UNSAFES_USER = (
    set(
        filter(
            bool,
            open(cwd / "localutils/unsafes/user.txt", "r", encoding="utf-8")
            .read()
            .split(),
        )
    )
